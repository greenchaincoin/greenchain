...
{
    QString ret = QString("Greenchain:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non gc unit input in lnReqAmount, we generate the URI with BKC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::gc, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
